...
export default function AddressRangeSlider() {
    ...
    const [actualRangeValue, setActualRangeValue] = useState(MARKS.find((mark) => mark.label === `${locationRange} km`)?.value || 0);
    ...
    useEffect(() => {
        setActualRangeValue(MARKS.find((mark) => mark.label === `${locationRange} km`)?.value || 0)
    }, [locationRange])


    return <div className="flex flex-col gap-8">
        <Text align="center">Proximité</Text>
        <Slider value={actualRangeValue} onChange={onChange} label={(val) => MARKS.find((mark) => mark.value === val)?.label} step={15}
