...
const AddressDropdown = () => {
    ...
    const [debouncedAddress, setDebouncedAddress] = useDebouncedState(enteredAddress, 500)
    ...
    const onAddressClick = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    return (
        <Popover opened={popoverOpen} width={200}>
            <Popover.Target>
                <TextInput label='Votre ville' value={enteredAddress} onChange={onInputChange}
                ...
                <Popover.Dropdown>
                    <div className="flex flex-col gap-1">
                        {addressList.map((address) => (
                            <div key={address.placeId} onClick={onAddressClick} className="cursor-pointer">
